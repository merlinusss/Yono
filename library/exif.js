  ...
  await new Promise((resolve, reject) => {
    ff(inputPath)
      .outputOptions([
        '-movflags', 'faststart',
        '-pix_fmt', 'yuv420p',
        '-vf', 'scale=trunc(iw/2)*2:trunc(ih/2)*2' // Biar video valid
      ])
      .toFormat('mp4')
      .save(outputPath)
      .on('end', resolve)
...
async function videoToWebp(media) {
  const isGif = media.toString('hex', 0, 4) === '47494638';
  const ext = isGif ? '.gif' : '.mp4';

  const tmpIn = path.join(tmpdir(), `${crypto.randomBytes(6).toString("hex")}${ext}`)
  const tmpOut = path.join(tmpdir(), `${crypto.randomBytes(6).toString("hex")}.webp`)

  fs.writeFileSync(tmpIn, media)

  await new Promise((resolve, reject) => {
    let command = ff(tmpIn);
    if (!isGif) command.inputFormat('mp4');
    command.addOutputOptions([
        "-vcodec", "libwebp",
        "-vf", "scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15," +
               "pad=320:320:-1:-1:color=white@0.0,split [a][b];[a] palettegen=reserve_transparent=on:transparency_color=ffffff [p];" +
               "[b][p] paletteuse",
        "-loop", "0",
        "-ss", "00:00:00",
        "-t", "00:00:05",
...
module.exports = { gifToWebp, imageToWebp, webpToMp4, webpToImage, videoToWebp, writeExifImg, writeExifVid, writeExif }
